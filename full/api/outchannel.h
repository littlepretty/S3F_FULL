...
public:
  ...
  Likewise, a per-write delay may or may not be included.  If it is not, zero is assumed.
	...
	/**
	 * Return the least delay among all cross-timeline mappings whose minimum delay is at least as large as the argument.
	 ...
	 */
	ltime_t min_sync_all_timeline_delay(ltime_t);
