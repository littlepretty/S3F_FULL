...
TimelineInterface::~TimelineInterface() {
	...
	if(isOpenVZEmu == true)
	{
		pthread_barrier_destroy(&emu_barrier);
		pthread_barrier_destroy(&emu_window_barrier);
	}
#endif
#else
	...
	{
		emu_barrier.destroy();
		emu_window_barrier.destroy();
	}
#endif
#endif
}

Interface::Interface(int num_timelines, int ltps, bool isOpenVZEmu, int openvz_emu_type) :
			__num_timelines(num_timelines), __clock(0), __log_ticks_per_sec(ltps)
{
  ...
  if(isOpenVZEmu == true)
  {
	  pthread_barrier_init(&__tli.emu_barrier, NULL, num_timelines);
	  pthread_barrier_init(&__tli.emu_window_barrier, NULL, num_timelines);
  }
  ...
  if(isOpenVZEmu == true)
  {
	  __tli.emu_barrier.init(num_timelines);
	  __tli.emu_window_barrier.init(num_timelines);
  }
#endif //#ifdef OPENVZ_EMULATION
#endif //#ifdef PTHREAD_BARRIER
	...
	pthread_mutex_init(&Timeline::emu_window_barrier_min_value_mutex, NULL);
#endif
   ...
   if(isOpenVZEmu == true)
   {
	  if(openvz_emu_type == 0)
		  __tli.sc = SimCtrl::get_simctrl(EMULATION_LOCAL);
	  else
		  __tli.sc = SimCtrl::get_simctrl(EMULATION_DISTRIBUTED);

	  __tli.isOpenVZEmu = true;
   }
   else
   {
#endif
 	  __tli.sc = NULL;
	  __tli.isOpenVZEmu = false;
#ifdef OPENVZ_EMULATION
   }
#endif
...
#ifdef OPENVZ_EMULATION
  //initialize openVZ emulation for the first time
  if(__tli.isOpenVZEmu == true)
  {
	  int return_code;

#ifdef OPENVZ_EMULATION_LOOKAHEAD
	  return_code = __tli.sc->emu_init(LOOKAHEAD_ANN);
#else
	  return_code = __tli.sc->emu_init(LOOKAHEAD_NONE);
#endif

	  if(return_code != 0)
	  {
		  printf("emu_init() error, error code = %s\n", SimCtrl::errcode2txt(return_code));
		  exit(-1);
	  }
	  else
	  {
	...
	for(unsigned int t=0; t<__timeline_threads.size(); t++ ) {
		Timeline* tl = __timeline_threads[t].get_timeline();
		tl->min_sync_all_timeline_delay(-1); //compute min channel delay (all channels) per timeline
...
ltime_t Interface::advance_body(next_action nxt_act, ltime_t t , stop_cond sc) { 
...
#ifdef OPENVZ_EMULATION
    // currently emulation_finish is called after one epoch
    // it is not the end of emulation and we can start emulation again for next epoch
	if(__tli.isOpenVZEmu == true)
	{
		int return_code = __tli.sc->emu_done();
		if (return_code)
		{
			printf("Interface::advance_body, emu_done() error, error code= %s\n",
				SimCtrl::errcode2txt(return_code));
			exit(0);
		}
	}
	...
	ltime_t utime = tim.tv_sec*1000000 + tim.tv_usec;

	tim = use_data.ru_stime;
	ltime_t stime = tim.tv_sec*1000000 + tim.tv_usec;
...
ltime_t Interface::sim_exc_time() {
	return( __end_run_time - __start_run_time );
}

ltime_t Interface::full_exc_time() {
