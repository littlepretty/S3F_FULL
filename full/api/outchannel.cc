...
bool OutChannel::write(Activation act, ltime_t write_delay, unsigned int pri) {
	...
	for(unsigned int i=0; i<end; i++) {
		...
		//delivery2 = delivery1 + __mapped_to[i].transfer_delay() + 1;
...
ltime_t OutChannel::mapto(InChannel* ic, ltime_t transfer_delay ) {
	...
	if( owner()->alignment()->get_state() != RUNNING) {
		...
		{
			owner()->alignment()->same_timeline_delay( transfer_delay );
		}
		// update all timeline delay as well, used for emulation vtime integration
		owner()->alignment()->all_timeline_delay( transfer_delay );
...
ltime_t OutChannel::min_sync_all_timeline_delay( ltime_t thrs) {
