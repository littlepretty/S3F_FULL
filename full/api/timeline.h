...
public:
	...
	/**
	 * Return the pointer of the TimelineInterface class. There is only one TimelineInterface*.
	 */
	TimelineInterface* get_timeline_interface() { return __interface_control; }
  ...
  //to support openVZ emulation lookahead
  ltime_t get_lookahead();
  void set_lookahead(ltime_t ve_lookahead_input);
...
protected:
	...
	/**
	 * Return the timestamp of the event with least timestamp of the two event-lists.
	...
	void         all_timeline_delay(ltime_t);

	void         same_timeline_delay(ltime_t);
	...
	ltime_t      min_sync_all_timeline_delay(ltime_t thrs);
	...
	/** Returns the value of __min_sync_all_timeline_delay */
	ltime_t      get_min_sync_all_timeline_delay()	{ return __min_sync_all_timeline_delay; }

	/** Returns the value of __min_sync_same_timeline_delay */
	ltime_t      get_min_sync_same_timeline_delay()	{ return __min_sync_same_timeline_delay; }
   ...
   bool logical_advance()
	...
	/**
	 *  Select which event_list of the two (__events and __ve_sink_events) has the next event.
	 *  Return the evt_list indicator (0 - none, 1 - events, 2 - __ve_sink_events) and the next_evt_time (-1 if none)
	 */
	void select_next_event(int &evt_list, ltime_t &evt_time);

	void sim_global_scheduler_one_epoch();

	void sim_emu_global_scheduler_one_epoch();

	ltime_t ve_proxy_entity_potential_impact_time();

	void manual_binding_thread_to_core();
...
private:
	...
	/** Minimum transfer delay among all cross-timeline channels in this timeline. */
	ltime_t          __min_sync_cross_timeline_delay;

	/** Minimum transfer delay among all channels in this timeline (non-cross-timeline and cross-timeline). */
	ltime_t          __min_sync_all_timeline_delay;

	/** Minimum transfer delay among all non-cross-timeline channels in this timeline. */
	ltime_t          __min_sync_same_timeline_delay;
	...
	/**
	 * Used for integrated with openVZ-based network emulation.
	 * Storing events that can affect the status of a VE,
	 * in particular, a packet received at a VE-proxy entity
	 * (should be delivery to the corresponding VE as soon as possible)
	 * currently EVTYPE_ACTIVATE and EVTYPE_EXEC_ACTIVATE events,
	 * whose inChannel is attached to VE-proxy entity, are put into this event list
	 * if __ve_sink_events is used, then __events + __ve_sink_events = all events
	 * Details in the 2012 PADS paper "Virtual Time Integration of Emulation and Parallel Simulation"
	 */
	STL_EventList    __ve_sink_events;
	...
	ltime_t t_prime; ///< time of first executed deliver-to-VE event in previous SSW

	#ifdef OPENVZ_EMULATION_LOOKAHEAD
	  ltime_t ve_lookahead; ///< min lookahead received from openVZ emulation after one emulation cycle
	#endif
...
public:
	...
	static pthread_mutex_t emu_window_barrier_min_value_mutex;
	static ltime_t emu_window_barrier_min_value;
#endif
};
