...
Timeline::Timeline (TimelineInterface* tli) :
	...
	__min_sync_all_timeline_delay = -1;
	__min_sync_same_timeline_delay = -1;
...
HandleCode Timeline::schedule(Process *proc, ltime_t t,
	...
	// can only directly schedule a Process if its owner is on this timeline
	...
	if (!e)
	{
		printf ("Timeline::schedule(), unable to allocate memory for TIMEOUT (probably) event\n");
		exit(0);
	}
...
HandleCode Timeline::schedule(InChannel *ic, ltime_t t,
	...
	//
	if(ic->is_ve_attached() == true)
	{
		__ve_sink_events.push( eptr );
	}
	else
	{
		__events.push ( eptr );
...
/* **************************************************************
    ...
    Return -1 if the __events and __ve_sink_events event-lists are empty,
    otherwise return the timestamp of the event with least timestamp
    of the two event-lists.

 *****************************************************************/

ltime_t Timeline::next_time() {
  // return -1 if there are no events
  if( __events.empty() && __ve_sink_events.empty() ) return -1;
  if( __events.empty() ) return (__ve_sink_events.top())->get_time();
  if( __ve_sink_events.empty() ) return (__events.top())->get_time();
  return min( (__ve_sink_events.top())->get_time(), (__events.top())->get_time() );
}
...
void* Timeline::thread_function() {
	...
	while(1) {
		...
		{
			//not accidentally process any events after the epoch end
		...
		// enter the loop of doing synchronization windows until the full simulation epoch is covered.
#ifdef OPENVZ_EMULATION
		if(__interface_control->isOpenVZEmu == true)
			sim_emu_global_scheduler_one_epoch();
		else
			sim_global_scheduler_one_epoch();
#else
		sim_global_scheduler_one_epoch();
#endif
	}
...
ltime_t Timeline::ve_proxy_entity_potential_impact_time()
{
	ltime_t nxt_time_ve;
	ltime_t nxt_time;
	ltime_t emu_now = __interface_control->sc->get_clock();
	ltime_t return_time;

	if( __events.empty() && __ve_sink_events.empty() )
	{
#ifdef OPENVZ_EMULATION_LOOKAHEAD
		//return emu_now + __min_sync_all_timeline_delay;
#else
		return emu_now + STOPBYDEMAND_MAX_ESW_SCALAR * VTIME_VTIMESLICE;
#endif
	}

	if( __ve_sink_events.empty() ) nxt_time_ve = INT_MAX;
	else nxt_time_ve = __ve_sink_events.top()->get_time();

	if( __events.empty() ) nxt_time = INT_MAX;
	else nxt_time = __events.top()->get_time();

#ifdef OPENVZ_EMULATION_LOOKAHEAD
	return_time = min(min(nxt_time, emu_now + ve_lookahead) + __min_sync_all_timeline_delay, nxt_time_ve);
	clear_lookahead();
#else
	return_time = min(emu_now + __min_sync_all_timeline_delay, nxt_time_ve);
#endif
...
#ifdef OPENVZ_EMULATION
void Timeline::sim_emu_global_scheduler_one_epoch()
{
	ltime_t epoch_end = __interface_control->get_stop_before();
	ltime_t nxt_time;
	int emu_window_barrier_status = 0;
	ltime_t now_emu; //current emulation time; while __time is the current simulation time
	ltime_t emu_cycle_run_time;
	__interface_control->sim_target_time = __time;

	int bottom_barrier_status = 0;

	do
	{
		now_emu = __interface_control->sc->get_clock();

#ifdef DEBUG
		printf("Decide emu or sim go next: sim_now = %ld, emu_now = %ld \n",
				__time, now_emu);
#endif
		if(__time+1 >= __interface_control->sim_target_time) //run emulation cycle
		{
			//step 1: compute Emulation Synchronization Window (ESW)
			//as for now, only two options: either lookahead, or stopByDemand (default)
			ltime_t p = ve_proxy_entity_potential_impact_time();

#ifdef MUTEX_BARRIER
			emu_window_barrier_status = __interface_control->emu_window_barrier.wait(p);
#endif
#ifdef SCHED_YIELD_BARRIER
			emu_window_barrier_status = __interface_control->emu_window_barrier.wait(s3fid(), p);
#endif
#ifdef PTHREAD_BARRIER
			pthread_mutex_lock(&emu_window_barrier_min_value_mutex);
			if(emu_window_barrier_min_value == -1 || emu_window_barrier_min_value > p) emu_window_barrier_min_value = p;
			pthread_mutex_unlock(&emu_window_barrier_min_value_mutex);
			emu_window_barrier_status = pthread_barrier_wait( &(__interface_control->emu_window_barrier) );
#endif
			if(emu_window_barrier_status == -1 || __interface_control->get_numTimelines() == 1)
			{
				//got the ESW
#ifdef PTHREAD_BARRIER
				emu_cycle_run_time = MAX( now_emu + VTIME_VTIMESLICE, emu_window_barrier_min_value);
				emu_window_barrier_min_value = -1; //reset emu_window_barrier_min_value for next ESW
#else
				emu_cycle_run_time = MAX( now_emu + VTIME_VTIMESLICE, __interface_control->emu_window_barrier.get_min_value() );
#endif

#ifdef DEBUG
				printf("thread_function(): run emulation \n");
				printf("sim_now = %ld, emu_now = %ld, emu_cycle_run_until = %ld, emu_cycle_run_time = %ld \n",
						__time, now_emu, emu_cycle_run_time, emu_cycle_run_time - __time);
#endif
				//step 2: run openVZ emulation for ESW
				// run the emulation cycle in the last simulation thread to reach emu_window_barrier

				int return_code;
#ifndef OPENVZ_EMULATION_LOOKAHEAD
				//set mindelay for each VE for stopByDemand case
				int idbase = __interface_control->sc->get_idbase();
				int nr_ve = __interface_control->sc->get_nve();
				for (int i = 0; i < nr_ve; i++)
				{
					return_code = __interface_control->sc->set_ve_mindelay(idbase+i, __min_sync_all_timeline_delay);
					if (return_code)
					{
						printf("error in set_ve_mindelay(), error code = %s\n", SimCtrl::errcode2txt(return_code));
						exit(-1);
					}
				}
#endif
				if(__interface_control->user_force_stop == false)
				{
					return_code = __interface_control->sc->emu_cycle(emu_cycle_run_time);
					if (return_code != 0)
					{
						printf("error in emu_cycle(), error code = %s\n", SimCtrl::errcode2txt(return_code));
						exit(-1);
					}
				}
				//step 3: inject the packets from emulator to simulator
				__interface_control->sc->inject_emu_event_to_s3fnet();

#ifdef OPENVZ_EMULATION_LOOKAHEAD
				//update lookahead offered by emulation
				int idbase = __interface_control->sc->get_idbase();
				int nr_ve = __interface_control->sc->get_nve();
				for(int i=0; i<nr_ve; i++)
				{
					VEHandle* vh = __interface_control->sc->get_vehandle(idbase+i);
					vh->tl->set_lookahead(vh->lookahead);
				}
				__interface_control->sim_target_time = ve_lookahead + __interface_control->sc->get_clock();
#else
				__interface_control->sim_target_time = __interface_control->sc->get_clock();
#endif

#ifdef DEBUG
				printf("thread_function(): completed an emulation cycle, "
						"now_emu_time = %ld\n", __interface_control->sc->get_clock());
#endif
			}

			//wait for the openVZ emulation to finish, achieved by the emu_barrier
#ifdef MUTEX_BARRIER
			__interface_control->emu_barrier.wait(-1);
#endif
#ifdef SCHED_YIELD_BARRIER
			__interface_control->emu_barrier.wait(s3fid(), -1);
#endif
#ifdef PTHREAD_BARRIER
			pthread_barrier_wait( &(__interface_control->emu_barrier) );
#endif
		}
		else //run simulation cycle
		{
			//step 1: schedule the cross timeline events
			// make this step 1 because the emulation is running first, which can generate cross timeline events
			get_cross_timeline_events();

			//step 2: compute SSW
			// equal to min{ sim_target_time,
			//      min_{all timelines}{ time_of_first_event + minimum_cross_timeline_delay },
			//	  min_{all_timelines}{ max (t_emu, time_of_first_event + minimum_all_timeline_delay)}

			// get next_event_time
			// no events in the timeline means that the timeline will
			// NOT affect other timeline until next emu_cycle,
			// when VE may generate events at the beginning of next emu cycle

			ltime_t emu_now = __interface_control->sc->get_clock();
			nxt_time = next_time();
			if(nxt_time < 0)  nxt_time = emu_now;

			if( __interface_control->get_numTimelines() > 1 && __min_sync_cross_timeline_delay > 0 )
			{
#ifdef MUTEX_BARRIER
				bottom_barrier_status = __interface_control->bottom_barrier.wait
						( MIN(nxt_time + __min_sync_cross_timeline_delay, MAX(nxt_time + __min_sync_all_timeline_delay, emu_now)) );
#endif
#ifdef SCHED_YIELD_BARRIER
				bottom_barrier_status = __interface_control->bottom_barrier.wait
						( s3fid(), MIN(nxt_time + __min_sync_cross_timeline_delay, MAX(nxt_time + __min_sync_all_timeline_delay, emu_now)) );
#endif
#ifdef PTHREAD_BARRIER
				pthread_mutex_lock(&bottom_barrier_min_value_mutex);
				ltime_t q = nxt_time + __min_sync_cross_timeline_delay;
				ltime_t r = MAX(nxt_time + __min_sync_all_timeline_delay, emu_now);
				ltime_t offered_min_time = min(q, r);
				if(bottom_barrier_min_value == prev_bottom_barrier_min_value || bottom_barrier_min_value > offered_min_time) bottom_barrier_min_value = offered_min_time;
...
#ifdef DEBUG
				printf("[multi-timeline], timeline %d, reach bottom_barrier (i.e. sync_win calculation):"
						" now = %ld, nxt_time = %ld, __min_sync_cross_timeline_delay = %ld\n",
						s3fid(), __time, nxt_time, __min_sync_cross_timeline_delay);
#endif

				// get the upper window edge computed by the min-reduction.
#ifdef PTHREAD_BARRIER
				__stop_before = min(__interface_control->sim_target_time, bottom_barrier_min_value);
#else
				__stop_before = min(__interface_control->sim_target_time, __interface_control->bottom_barrier.get_min_value());
#endif
				__stop_before = min(__stop_before, epoch_end);

#ifdef DEBUG
#ifdef PTHREAD_BARRIER
				ltime_t bbmv = bottom_barrier_min_value;
#else
				ltime_t bbmv = __interface_control->bottom_barrier.get_min_value();
#endif
				printf("[multi-timeline], timeline %d, __stop_before = %ld, "
						"bottom_barrier min value = %ld, emu_now = %ld, epoch_end = %ld, sim_target_time = %ld\n",
						s3fid(), __stop_before, bbmv, emu_now, epoch_end, __interface_control->sim_target_time);
#endif
			}
			else  //this is for single timeline or no cross-timeline events cases
			{
				__stop_before = min(__interface_control->sim_target_time, max(next_time()+__min_sync_cross_timeline_delay, emu_now));
				__stop_before = min(__stop_before, epoch_end);
			}

			//step 3: run simulation for SSW
			change_state(RUNNING);

#ifdef DEBUG
			printf("timeline %d enters window [%ld,%ld), size = %ld\n",
					s3fid(), __time, __stop_before, __stop_before - __time);
#endif
			t_prime = 0;
			sync_window(); //do the actual simulation work
			//update sim_target_time
			pthread_mutex_lock(&__interface_control->sim_target_time_mutex);
			__interface_control->sim_target_time = min(__interface_control->sim_target_time, max(__time, max(t_prime, emu_now)));
			pthread_mutex_unlock(&__interface_control->sim_target_time_mutex);
			...
			//
			if( __stop_before < epoch_end && __interface_control->user_force_stop == false) // do another synchronization window
			{
				if( __interface_control->get_numTimelines() > 1 && __min_sync_cross_timeline_delay > 0 )
				{
#ifdef MUTEX_BARRIER
	...
	} while( __stop_before < epoch_end && __interface_control->user_force_stop == false);
}
#endif //#ifdef OPENVZ_EMULATION


void Timeline::sim_global_scheduler_one_epoch()
{
	ltime_t epoch_end = __interface_control->get_stop_before();
	ltime_t nxt_time;
	...
	do
	{
		...
		change_state(BLOCKED);
		nxt_time = next_time();
#ifdef FAST_MULTITIMELINE_NO_X_EVENTS
		//todo revisit this part if cross-timeline delay is dynamic, e.g. support dynamic routing
		if( __interface_control->get_numTimelines() > 1 && __min_sync_cross_timeline_delay > 0 )
#else
...
#endif
		{
			...
			if( nxt_time < 0 || __min_sync_cross_timeline_delay < 0  )
			{
				...
				{
#ifdef MUTEX_BARRIER
					bottom_barrier_status = __interface_control->bottom_barrier.wait( this_window );
#endif
#ifdef SCHED_YIELD_BARRIER
					bottom_barrier_status = __interface_control->bottom_barrier.wait( s3fid(), this_window );
#endif
#ifdef PTHREAD_BARRIER
					pthread_mutex_lock(&bottom_barrier_min_value_mutex);
				...
				else //mean nothing to execute, next_time should be infinitely large
				{
#ifdef MUTEX_BARRIER
					bottom_barrier_status = __interface_control->bottom_barrier.wait(-1);
#endif
#ifdef SCHED_YIELD_BARRIER
					bottom_barrier_status = __interface_control->bottom_barrier.wait(s3fid(), -1);
#endif
#ifdef PTHREAD_BARRIER
					bottom_barrier_status = pthread_barrier_wait( &(__interface_control->bottom_barrier) );
#endif
...
#ifdef MUTEX_BARRIER
				bottom_barrier_status = __interface_control->bottom_barrier.wait( MAX(__time,nxt_time) + MAX(0,__min_sync_cross_timeline_delay));
#endif
#ifdef SCHED_YIELD_BARRIER
				bottom_barrier_status = __interface_control->bottom_barrier.wait( s3fid(), MAX(__time,nxt_time) + MAX(0,__min_sync_cross_timeline_delay));
#endif
#ifdef PTHREAD_BARRIER
		...
		//   While this is a min-reduction, we aren't using that capability
		// here, so just offer -1 (nothing)
		if( __stop_before < epoch_end && __interface_control->user_force_stop == false)
		{
#ifdef MUTEX_BARRIER
			__interface_control->top_barrier.wait(-1);
#endif
...
void Timeline::sync_window() {
	...
	// data structures
	EventPtr   nxt_evt;
	int selected_evtlst; // 0 - none, 1 - events, 2 - __ve_sink_events
	ltime_t nxt_evt_time;
	Entity*  ent;
	OutChannel* oc;
	InChannel* ic;
	ltime_t delay;
	...
	select_next_event(selected_evtlst, nxt_evt_time);

	// event list loop.  Do all events with times strictly less than __stop_before
	//
	while ( selected_evtlst != 0 && nxt_evt_time < __stop_before && __interface_control->user_force_stop == false)
	{
		if(selected_evtlst == 2)
		{
			nxt_evt = __ve_sink_events.top();
			__ve_sink_events.pop();
			if(t_prime == 0) t_prime = nxt_evt->get_time(); //set t_prime, the time of first executed deliver-to-VE event in current SSW
		}
		else //selected_evtlst == 1
		{
...
#ifdef DEBUG
		printf("At %ld on timeline %d event type %d executed, selected_evtlst = %d\n",
				__time, s3fid(), nxt_evt->get_evtype(), selected_evtlst);
#endif
		...
		case EVTYPE_EXEC_ACTIVATE:
			...
			// running process needs to know what inchannel triggered it
			//
		...
		case EVTYPE_WAIT_APPT:
			...
			// move any events left here by the appointment source into own event list
			{
				...
				for(int i = out_appt->events.size()-1; i>= 0; i--) {
					InChannel* ic = out_appt->events[i]->get_inc();
					assert(ic);

					if( ic->is_ve_attached() == true &&
							(out_appt->events[i]->get_evtype() == EVTYPE_EXEC_ACTIVATE ||
									out_appt->events[i]->get_evtype() == EVTYPE_ACTIVATE) )
					{
						__ve_sink_events.push( out_appt->events[i] );
					}
					else
					{
		...
		//prepare for processing the next event
		select_next_event(selected_evtlst, nxt_evt_time);
	}
...
void Timeline::select_next_event(int &evt_list, ltime_t &evt_time)
{
	if( __events.empty() && __ve_sink_events.empty() )
	{
		evt_list = 0;
	...
	{
		evt_list = 2;
		evt_time = (__ve_sink_events.top())->get_time();
		return;
	}

	if( __ve_sink_events.empty() )
	{
		evt_list = 1;
		evt_time = (__events.top())->get_time();
		return;
	}

	EventPtr lhs =  __events.top();
	EventPtr rhs =  __ve_sink_events.top();
	int who = 1; //1 for __events and 2 for __ve_sink_events

	if( lhs->__time > rhs->__time ) who = 2;
	else if( lhs->__time < rhs->__time ) who = 1;
	else if( lhs->__key2 > rhs->__key2)  who = 2;
	else who = 1;
...
void Timeline::get_cross_timeline_events() {
	...
	// For each buffered event, all that is needed is a copy into __events
	//

	for(int i=0; i<num_to_do; i++) {
		InChannel* ic = __window_list[i]->get_inc();

		if(!ic) break;

		if( ic->is_ve_attached() == true &&
				(__window_list[i]->get_evtype() == EVTYPE_EXEC_ACTIVATE ||
						__window_list[i]->get_evtype() == EVTYPE_ACTIVATE) )
		{
			__ve_sink_events.push( __window_list[i] );
		}
		else
		{
...
Timeline:: ~Timeline() {  
	...
	if( !s3fid() ) pthread_mutex_destroy(&emu_window_barrier_min_value_mutex);
#endif
...
void Timeline::all_timeline_delay(ltime_t delay) {
	if( (__min_sync_all_timeline_delay == -1) ||
			(delay < __min_sync_all_timeline_delay) ) {
		__min_sync_all_timeline_delay = delay;
	}
}

void Timeline::same_timeline_delay(ltime_t delay) {
	if( (__min_sync_same_timeline_delay == -1) ||
			(delay < __min_sync_same_timeline_delay) ) {
		__min_sync_same_timeline_delay = delay;
	}
}
...
ltime_t Timeline::min_sync_all_timeline_delay(ltime_t thrs) {
	...
	//
	for(unsigned int i=0; i<end; i++) {
		ent_delay = __entity_list[i]->min_sync_all_timeline_delay(thrs);
		if(ent_delay != -1) // -1 means everything is on the same timeline
		{
			if( thrs<= ent_delay && ((ans<0) || (ent_delay<ans))) ans = ent_delay;
		}
...
#ifdef OPENVZ_EMULATION_LOOKAHEAD
ltime_t Timeline::get_lookahead()
{
#ifdef DEBUG
	printf("Timeline::get_lookahead(): timeline %d, next_event_from_now = %ld, ve_lookahead = %ld\n",
			s3fid(), next_time()-__time, ve_lookahead);
#endif

	if(ve_lookahead < 0) //all entities are non-VE hosts
		return MAX(__time, next_time());
	else if (next_time() < 0)
		return __time+MAX(0, ve_lookahead);
	else return min(next_time(), __time+ve_lookahead);
}

void Timeline::set_lookahead(ltime_t ve_lookahead_input)
{
	if(ve_lookahead == -1 || ve_lookahead > ve_lookahead_input)
		ve_lookahead = ve_lookahead_input;
}

void Timeline::clear_lookahead()
{
	ve_lookahead = -1;
}
#endif
...
pthread_mutex_t Timeline::emu_window_barrier_min_value_mutex;
ltime_t Timeline::emu_window_barrier_min_value = -1;
#endif
