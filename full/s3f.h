...
#if defined(macintosh) || defined(Macintosh) || defined(__APPLE__) || defined(__MACH__) || defined(_WIN32) || defined(_WIN64)
#error OpenVZ-based emulation and OpenFlow simulation only work on Linux platform.
#endif
...
#error COMPOSITE_SYNC does not support openvz-based emulation. Pls turn off OPENVZ_EMULATION.
#endif
#endif

/**
 * \def SYNC_WIN_EVENT_COUNT
 * print out events per sync window per timeline
 * warning: lots of messages if sync windows are small
 */
//#define SYNC_WIN_EVENT_COUNT

/**
 * \def OPENVZ_EMULATION_LOOKAHEAD
 * whether the emulation lookahead (an application-level neural-network-based lookahead model) is used
 */
//#define OPENVZ_EMULATION_LOOKAHEAD
#ifdef OPENVZ_EMULATION_LOOKAHEAD
#ifndef OPENVZ_EMULATION
#error OPENVZ_EMULATION_LOOKAHEAD is defined, but OPENVZ_EMULATION is undefined.
#endif
#endif

// STOPBYDEMAND_MAX_ESW_SCALAR * timeslice is the max ESW, needed for openvz-based emulation
#define STOPBYDEMAND_MAX_ESW_SCALAR 1000

// if no cross-timeline events occur in multi-timeline experiments, treat it like single timeline for performance gain
//#define FAST_MULTITIMELINE_NO_X_EVENTS

// -1 is sometimes used to flag an error when an ltime_t value
// is returned, or an ltime_t value to ignore.  So we do not 
// use unsigned long
// 
