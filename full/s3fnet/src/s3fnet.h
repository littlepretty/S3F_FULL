...
#define OPENFLOW_SWITCH_PROTOCOL_NAME  "ofswitch"
/** OpenFlow controller protocol layer session name. */
#define OPENFLOW_CONTROLLER_PROTOCOL_NAME  "ofctrl"
...
enum S3FNetProtocolType {
  ...
  /** An OpenFlow switch protocol */
  S3FNET_PROTOCOL_TYPE_OPENFLOW_SWITCH = 241,

  /** An OpenFlow switch protocol */
  S3FNET_PROTOCOL_TYPE_OPENFLOW_CONTROLLER = 242,
};
...
enum ProtocolSessionCtrlTypes {
   ...
   */
  OPENFLOW_SWITCH_CTRL_COMMAND1		= 200,
  OPENFLOW_SWITCH_CTRL_COMMAND2		= 201,
                
  /*
   * openflow controller protocol control commands //not in use yet
   */
  OPENFLOW_CONTROLLER_CTRL_COMMAND1		= 202,
  OPENFLOW_CONTROLLER_CTRL_COMMAND2		= 203,
};
...
class SimInterface : public Interface {
 public:
	/** the constructor
	    ...
	    * @param isOpenVZEmu whether the OpenVZ-based emulation is supported. The true/false value is passed through DML (one of the top few lines).
	    * @param openvz_emulation_type indicates the type of openvz-based emulation, local (single physical machine) or distributed.
	    * @param of_ctrl_type indicates the type of simulated openflow controller.
	    * 		 0 means non-entity-based controller (default), interacting with simulated openflow switches through function calls;
	    *        1 means entity-based controller that aligned to a timeline like other S3F entity, and controller is a protocol session.
	    * @param of_ctrl_model indicates the model of simulated non-entity-based openflow controller.
	    *        0 means no controller
	    *        1 means OF_LEARNING_SWITCH_CONTROLLER
	    *        refer openflow_interface.h for more configuration
	    */
   SimInterface(int tl, int ltps, bool isOpenVZEmu, int openvz_emulation_type, int of_ctrl_type = 0, int of_ctrl_model = 0) :
	   Interface(tl,ltps, isOpenVZEmu, openvz_emulation_type), openflow_controller_type(of_ctrl_type), openflow_controller_model(of_ctrl_model) {}
   ...
   int openflow_controller_type;

   int openflow_controller_model;
};
