...
/** The auxiliary object pop up together with the packet from MAC layer to upper layer */
class MACOptionToAbove {
  public:
	/** the network interface that receives the packet */
	int nw_iface;

	/** Protocol identifier for the protocol above MAC. */
	uint8 protocol_no;
};

/** The auxiliary object pop up together with the packet from PHY layer to upper layer */
class PHYOptionToAbove {
  public:
   /** the network interface that receives the packet */
   int nw_iface;	
};
