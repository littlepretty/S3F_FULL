...
void TCPMaster::fast_timer_callback(Activation ac)
{
    ...
    tm->fast_timer_callback_body(ac);
}

void TCPMaster::fast_timer_callback_body(Activation ac)
{
  delete_defunct_sessions();
  ...
  int mysize;
  if((mysize = fast_timeout_sessions.size()) > 0)
  {
    TCPSession** mylist  = new TCPSession*[mysize];
    assert(mylist);
    int idx = 0;
    for(S3FNET_SET(TCPSession*)::iterator iter = fast_timeout_sessions.begin(); iter != fast_timeout_sessions.end(); iter++, idx++)
      mylist[idx] = (*iter);
  ...
  double time = inHost()->t2d(getNow(), 0);
  double t = calc_fast_timeout(time);
  HandleCode h = inHost()->waitFor( fast_timer_callback_proc, ac, inHost()->d2t(t, 0), inHost()->tie_breaking_seed );
}

void TCPMaster::slow_timer_callback(Activation ac)
{
	TCPMaster* tm = (TCPMaster*)((ProtocolCallbackActivation*)ac)->session;
    tm->slow_timer_callback_body(ac);
}

void TCPMaster::slow_timer_callback_body(Activation ac)
{
  delete_defunct_sessions();
  ...
  int mysize;
  if((mysize = connected_sessions.size()) > 0)
  {
    TCPSession** mylist  = new TCPSession*[mysize];
    assert(mylist);
    int idx = 0;
    for(S3FNET_SET(TCPSession*)::iterator iter = connected_sessions.begin(); iter != connected_sessions.end(); iter++, idx++)
      mylist[idx] = (*iter);
    for(idx = 0; idx < mysize; idx++)
  ...
  HandleCode h = inHost()->waitFor( slow_timer_callback_proc, ac, inHost()->d2t(t, 0), inHost()->tie_breaking_seed );
}
