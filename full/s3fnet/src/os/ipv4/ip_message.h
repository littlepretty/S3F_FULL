...
class IPMessage: public ProtocolMessage {
 public:
  ...
  IPMessage(IPADDR src, IPADDR dest, uint8 protono, uint8 live, uint16 id, uint8 tos_ip, uint8 flags_ip, uint16 fragmentOffset_ip, uint16 total_length_ip);
  ...
  virtual int packingSize()
  {
    ...
    return S3FNET_IPHDR_LENGTH + ProtocolMessage::packingSize();
  }
  ...
  uint8 tos; //Also used as DSCP + ECN value
  uint8 flags;
