...
#ifndef __RANDOM_DUMMY_SESSION_H__
#define __RANDOM_DUMMY_SESSION_H__
...
#define RANDOM_DUMMY_PROTOCOL_CLASSNAME "S3F.OS.RANDOM_DUMMY"

/**
 * \brief A dummy protocol session, sending hello messages to specified peers.
 */
class RandomDummySession : public ProtocolSession {

 public:
  /** the constructor
   *  must be take an argument of the protocol graph
   */
  RandomDummySession(ProtocolGraph* graph);

  /** the destructor */
  virtual ~RandomDummySession();

  /** each protocol must have a unique protocol number */
  virtual int getProtocolNumber() { return S3FNET_PROTOCOL_TYPE_RANDOM_DUMMY; }
...
#endif /*__RANDOM_DUMMY_SESSION_H__*/
