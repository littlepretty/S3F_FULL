  ...
  veid2hostobj_map(0)
{
  ...
  veid2hostobj_map = new VEID2HOSTOBJ_MAP;
}
...
NameService::~NameService()
{
  ...
  if(veid2hostobj_map) delete veid2hostobj_map;
}
...
Host* NameService::veid2hostobj(int id)
{
	if(!veid2hostobj_map) return 0;

	VEID2HOSTOBJ_MAP::iterator iter = veid2hostobj_map->find(id);
	if(iter == veid2hostobj_map->end()) return 0;
	else return ((*iter).second);
}

int NameService::ip2veid(IPADDR addr)
{
	Host* host = ip2hostobj(addr);
	if(!host) return -1;
	else return host->getVEID();
}

int NameService::hostnhi2veid(const S3FNET_STRING& nhi)
{
	IPADDR ip = nhi2ip(nhi);
	if(ip == IPADDR_INVALID) return 0;
	else return ip2veid(ip);
}

void NameService::print_veid2hostobj_map()
{
	printf("print_veid2hostobj_map()\n");
	VEID2HOSTOBJ_MAP::iterator iter;
	for (iter = veid2hostobj_map->begin(); iter != veid2hostobj_map->end(); iter++)
	{
		printf("veid = %d, host = %p\n",
				(int)((*iter).first),
				(Host*)((*iter).second));
	}
}
