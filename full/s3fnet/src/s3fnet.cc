...
{
	if (sim_inf->get_timeline_interface()->user_force_stop) exit(0);
	sim_inf->get_timeline_interface()->user_force_stop = true;
	printf("\ntrying to stop, press ctrl+c again to force exit\n");
}
...
int main(int argc, char** argv)
{
  ...
  int seed;
  bool isOpenVZEmu;
  int openvz_emulation_type; // 0 - EMULATION_LOCAL; 1 - EMULATION_DISTRIBUTED
  int of_controller_type; // 0 - non-entity-based controller; 1 - entity-based controller
  int of_controller_model; // refer to enum OPENFLOW_CONTROLLER_MODEL in openflow_interface.h
  ...
  str = (char*)dml_cfg->findSingle("enable_openvz_emulation");
  if(str)
  {
    if(s3f::dml::dmlConfig::isConf(str))
      error_quit("ERROR: invalid enable_openvz_emulation attribute.\n");
    if(!strcasecmp(str, "true")) isOpenVZEmu = true;
    else isOpenVZEmu = false;
  }
  else isOpenVZEmu = false;
#ifndef OPENVZ_EMULATION
  if (isOpenVZEmu == true)
	  error_quit("ERROR: isOpenVZEmu is set to true in DML, but OPENVZ_EMULATION is not defined.\n");
#endif

  str = (char*)dml_cfg->findSingle("openvz_emulation_type");
  if(!str) openvz_emulation_type = 0;
  else
  {
    if(s3f::dml::dmlConfig::isConf(str))
      error_quit("ERROR: invalid openvz_emulation_type attribute.\n");
    openvz_emulation_type = atoi(str);
    if(openvz_emulation_type != 0 && openvz_emulation_type != 1)
      error_quit("ERROR: openvz_emulation_type attribute must be 0 (EMULATION_LOCAL) or 1 (EMULATION_DISTRIBUTED).\n");
  }

  str = (char*)dml_cfg->findSingle("simulated_openflow_controller_type");
  if(!str) of_controller_type = 0;
  else
  {
    if(s3f::dml::dmlConfig::isConf(str))
      error_quit("ERROR: invalid simulated_openflow_controller_type attribute.\n");
    of_controller_type = atoi(str);
    if(of_controller_type != 0 && of_controller_type != 1)
      error_quit("ERROR: simulated_openflow_controller_type attribute must be 0 (NON-ENTITY-BASED CONTROLLER) or 1 (ENTITY-BASED CONTROLLER).\n");
  }

  str = (char*)dml_cfg->findSingle("simulated_openflow_controller_model");
  if(!str) of_controller_model = 0;
  else
  {
    if(s3f::dml::dmlConfig::isConf(str))
      error_quit("ERROR: invalid simulated_openflow_controller_model attribute.\n");
    of_controller_model = atoi(str);
    if(of_controller_model < 0)
      error_quit("ERROR: simulated_openflow_controller_model attribute must be a non-negative integer.\n");
  }

  MAIN_DUMP(printf("total_timeline = %d, tick_per_second= %d, "
		  "run_time = %ld, seed = %d, enable_openvz_emulation = %d, "
		  "openvz_emulation_type = %d, simulated_openflow_controller_type = %d,"
		  "simulated_openflow_controller_model = %d \n",
		  total_timeline, tick_per_second, sim_single_run_time,
		  seed, isOpenVZEmu, openvz_emulation_type, of_controller_type, of_controller_model));
  ...
  sim_inf = new SimInterface( total_timeline, tick_per_second, isOpenVZEmu, openvz_emulation_type, of_controller_type, of_controller_model );

  //build, configure and initialize the simulation model
  ...
  // delete may cause a segmentation fault, flush buffers
  fflush(stdout);
