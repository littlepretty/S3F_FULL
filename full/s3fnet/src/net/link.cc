...
Link::Link(Net* theNet) : DmlObject((DmlObject*)theNet), delay(0), owner_net(theNet), rdrop_rate(0)
{
...
void Link::config(s3f::dml::Configuration* cfg)
{
  ...
  str = (char*)cfg->findSingle("rdrop_rate");
  if(str)
  {
	  if(s3f::dml::dmlConfig::isConf(str))
		  error_quit("ERROR: Link::config(), missing or invalid link.rdrop_rate attribute.\n");

	  rdrop_rate = atof(str);
	  if(rdrop_rate < 0 || rdrop_rate > 1)
		  error_quit("ERROR: Link::config(), link.prop_delay is within the range [0, 1].");
  }
  else
  {
	  rdrop_rate = 0;
