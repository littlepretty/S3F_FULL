...
Host::Host(Timeline* tl, Net* parent, long hostid) : Entity(tl), DmlObject(parent, hostid),
		host_seed(0), is_router(false), is_switch(false), network_prot(0), is_openVZEmu(false),
		is_of_switch(false), is_of_ctrl(false), of_ctrl_sess(0), of_ctrl_type(1)
{
...
void Host::config(s3f::dml::Configuration* cfg)
{
	...
	char* str = (char*)cfg->findSingle("veid");
	if(!str)
	{
		veid = -1;
		is_openVZEmu = false;
	...
	else
	{
		if(s3f::dml::dmlConfig::isConf(str))
			error_quit("ERROR: invalid veid attribute.\n");
		veid = atoi(str);
		is_openVZEmu = true;
		if(veid < 0)
			error_quit("ERROR: veid attribute must be a non-negative integer.\n");
	}
#ifndef OPENVZ_EMULATION
	if(is_openVZEmu == true)
		error_quit("ERROR: host's veid is set, but OPENVZ_EMULATION is undefined.\n");
#endif
	HOST_DUMP(printf("nhi=\"%s\", veid=%d.\n", nhi.toString(), veid));

	HOST_DUMP(printf("[nhi=\"%s\"] register host.\n", nhi.toString()));
	dynamic_cast<Net*>(myParent)->register_host(this, S3FNET_STRING(nhi.toStlString()), is_openVZEmu);

	str = (char*)cfg->findSingle("rng_level");
	if(str)
	{
		if(s3f::dml::dmlConfig::isConf(str))
	...
	bool is_first_iface = true; //to ensure the interface start from 0 for openvz
	while(ss->hasMoreElements())
	{
		...
		if(is_first_iface == true)
		{
			is_first_iface = false;
			if(low !=0 && is_openVZEmu == true)
				error_quit("VE's interface must start from 0\n");
		}
	...
	if(is_openVZEmu == true)
	{
		int idbase = inNet()->getTopNet()->get_idbase();
		SimCtrl* simCtrl = inNet()->getTopNet()->getSimCtrl();

		//set #iface in VE
		int return_code = simCtrl->set_ve_nif(idbase+veid, ifaces.size());
		if(return_code != 0)
		{
			error_quit("Host::config(), simCtrl->set_ve_nif error, return code = %d\n", return_code);
		}
	}
#endif
...
void Host::init()
{
...
#ifdef OPENVZ_EMULATION
	if(is_openVZEmu == true) //set the alignment in this VE's VEhandler, to support openVZ emulation lookahead
	{
		int idbase = inNet()->getTopNet()->get_idbase();
		SimCtrl* simCtrl = inNet()->getTopNet()->getSimCtrl();
		assert(simCtrl);
		VEHandle* vh = simCtrl->get_vehandle(idbase+veid);
		assert(vh);
		vh->tl = alignment();
	}
#endif
	...
	else nDigits = (int)(floor(log10(abs(nhi.ids[1]))) + 1);
	tie_breaking_seed = (long)(nhi.ids[0]*pow(10.0, (double)max(6, nDigits)) + nhi.ids[1]);
}
