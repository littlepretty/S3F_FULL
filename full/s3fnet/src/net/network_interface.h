...
public:
	...
	//support openflow controller protocol session
	/** Indicate whether the network interface is the one connecting to an openflow controller, only an openflow switch has such interface */
	bool is_of_ctrl_iface;
	/** Pointer to the OpenFlowSwitchSession protocol session layer */
	OpenFlowSwitchSession* of_switch_sess;
	/** Handle the message from openflow switch to openflow controller */
	void handle_openflow_message_to_controller(Activation pkt);

	/** Indicate if the promiscuous mode is enabled */
	bool isPromiscuousMode() { return is_promiscuous; }
...
protected:
	...
	/**
	 * Modify packet for openvz emu.
	 * In particular, add the network interface id through which the packet is received into the emulation packet.
	 */
	void emu_packet_modify_iface(Activation pkt);
	...
	/** whether the network interface uses promiscuous mode */
	bool is_promiscuous;
};
