/**
 * \file trie4.h
 * \brief Header file for the ip-optimized Trie4 class (original Trie with minor fixes).
 *
 ...
 *
 * This class directly extends Trie1 adding coalescing of duplicate RouteInfo
 * structs into an array. The index into the array (vs the actual pointer) is
 * now stored in the Trie nodes. This class is intended primarily for testing
 * purposes as the neccessity of a base-class pointer-cast to TrieDataArray
 * adds at least one extra unnecessary memory access.
 *
 ...
 */
class Trie4 : public Trie {
 public:
  /** The constructor. */
  Trie4(int array_version = -1);
  ...
  * Creates a node from the root.
   ...
   */
  void create(TrieNode* theroot, TRIEBITSTRING key, 
  ...
  */
  ...
  /**
  * Data array that nodes index into.
  * Base class is located in trie.h but this will store a derived class.
  **/
  TrieDataArray* mData;
 ...
 public:
  ...
  public:
    ...
    */
    ...
    /**
    * iterator constructor for Trie4 types
    */
    iterator(Trie4* t);
    ...
    */
    iterator(const Trie4::iterator& it); // copy costr
...
class TrieNode {
 public:
  ...
  /**
  * The destructor.
  * All dealloc responsibility now lies with the Trie itself.
  */
  ~TrieNode() {};

  /** Index into Trie's Data array. */
  unsigned int index;
